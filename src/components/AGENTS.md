...
| 디렉토리 | 설명 |
|-----------|---------|
| `ui/` | 특정 기능에 속하지 않는 범용 재사용 프리미티브 (예: `bottom-sheet.tsx`, `text-field.tsx`, `tapbar.tsx`, `schedule-card.tsx`) |
| `home/` | 홈 화면 전용 컴포넌트 (`home-screen.tsx` = `/`) |
| `course/` | 코스 탭 전용 컴포넌트 (`course-screen.tsx` = `/course`, Upcoming/History 세그먼트 탭 + 탭별 코스 카드 리스트/공유 빈 상태 + FAB) |
...
- `ui/schedule-card.tsx`는 D-DAY 뱃지 + 코스명 + 날짜/스팟 개수 + Edit Schedule/View Course 버튼으로 된 코스 카드(Figma `card_schedule`)입니다. 원래 `home-screen.tsx`에 로컬로 있었는데 `course-screen.tsx`(`/course`)가 두 번째 사용처가 되면서 `ui/`로 옮겼습니다 — 두 화면 모두 `Schedule` 타입과 `ScheduleCard`를 가져다 씁니다. 버튼은 아직 `/course/[id]`나 `/course/[id]/edit` 라우트가 없어 정적 상태입니다.
- `course/course-screen.tsx`(`/course`)는 Upcoming/History 세그먼트 탭(로컬 state)을 가집니다. 탭 밑줄은 Figma 구조(`flex-[1_0_0]`로 텍스트 영역을 늘려 2px 바가 탭 바닥에 붙는 방식)를 그대로 따라야 합니다 — 텍스트+바를 통째로 `justify-center`로 가운데 정렬하면 바가 바닥에서 떠서 디자인과 어긋납니다. Upcoming 탭(Figma 노드 `137:1121`/`354:7845`)은 `UPCOMING_COURSES`(정적 스텁 배열, 코스 API 없음)가 비어 있으면 `EmptyState`를, 있으면 `ScheduleCard` 리스트를 보여줍니다. History 탭(노드 `354:7785`/`354:7899`)은 `HISTORY_COURSES`가 비어 있으면 같은 `EmptyState`를, 있으면 로컬 `HistoryCard`(뱃지 없이 "View Course" 버튼 1개짜리 카드, `ScheduleCard`와 구조가 달라 별도 컴포넌트로 둠) 리스트를 보여줍니다. 우하단 FAB(+, `PlusIcon`)는 `/course/create` 라우트가 없어 아직 onClick이 없습니다. FAB는 데스크톱에서도 `max-w-sm` 중앙 정렬 폭을 벗어나지 않도록 `fixed inset-x-0 mx-auto max-w-sm` 래퍼 안에 둡니다 — `ui/bottom-sheet.tsx`가 쓰는 것과 같은 패턴입니다. 코스 상세(카드의 View Course 클릭 시 이동하는 화면)는 스코프 밖입니다.
