...
- `course/course-detail-screen.tsx`(`/course/[id]`, Figma 노드 `358:10841`)는 뒤로가기(`router.back()`) + "Course name" 헤더(코스별 실제 이름을 가져올 API가 없어 Figma의 플레이스홀더 문구를 그대로 씀) + 지도/삭제 아이콘(아직 미연결) + 세로 점선 타임라인입니다. 타임라인은 `border-l border-dashed`로 그은 세로선 위에 각 정거장을 `bg-secondary-300` 원형 마커 + `ui/spot-card.tsx`로 배치하고, 점선의 `left` 값은 마커(`size-2.5`)의 실제 중심 x좌표(행의 `pl-[11px]` + 마커 반지름)와 맞춰야 합니다 — 그냥 마커 왼쪽 끝에 맞추면 선이 마커 중심에서 살짝 벗어나 보입니다. 정거장 사이에는 코스 자체의 이동 거리(각 스팟 카드 자신의 `distanceKm`—사용자 현재 위치 기준—와는 다른 값)를 텍스트로 넣습니다. 코스-스팟 연결 API가 없어 `COURSE_STOPS`는 `SPOTS[0]`을 3번 반복하는 정적 스텁입니다 — 실제 코스별 스팟 목록 API가 생기면 `id` 라우트 파라미터로 조회하도록 바꾸세요.
