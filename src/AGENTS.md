...
| `lib/` | 화면에 속하지 않는 순수 유틸/클라이언트 헬퍼. `lib/api/`에 백엔드 호출 함수(도메인별로 1파일 — `auth.ts` Auth 도메인 11개 엔드포인트 전부, `user.ts` User 도메인 5개 엔드포인트 전부, `course.ts` Course 도메인 5개 엔드포인트 전부), `client.ts`(공통 응답 봉투 처리 + zod 파싱을 하는 `apiFetch`), `auth-error-messages.ts`/`course-error-messages.ts`(도메인별 에러 코드 → 화면 문구, 여러 화면이 공유). `auth-tokens.ts`는 로그인 토큰 localStorage 저장/조회/삭제. 회원가입·비밀번호 재설정 위저드 상태는 별도 lib 파일 없이 각 오케스트레이터 라우트(`src/app/login/email/page.tsx`/`src/app/login/reset/email/page.tsx`)의 로컬 state로 직접 관리함 — `src/app/AGENTS.md`의 `login/` 항목 참고 |
