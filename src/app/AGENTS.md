...
| `mypage/`       | 마이페이지 라우트 — `/mypage`(하단 탭바 포함, 프로필 행 클릭 시 `/mypage/account`로 이동), `/mypage/account`(프로필 편집/로그아웃/계정 삭제). 화면 컴포넌트는 `src/components/mypage/`에 있고 각 `page.tsx`는 그걸 렌더링만 함. 하단 탭바(`Tapbar`)는 `mypage-screen.tsx` 안에 로컬 컴포넌트로만 있음 — Home/Course 라우트가 생겨 두 번째 사용처가 필요해지면 `components/ui/`로 옮길 것 |
