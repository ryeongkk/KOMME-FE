...
| `course/`       | 코스 라우트 — `/course`(하단 탭바 포함). Upcoming/History 세그먼트 탭(로컬 state) + 우하단 FAB(+, `/course/create` 라우트 없어 아직 미연결). Upcoming 탭(Figma 노드 `137:1121`/`354:7845`)은 코스 카드 리스트(`ui/schedule-card.tsx` 재사용, D-DAY 뱃지 + Edit Schedule/View Course 버튼 2개)를, History 탭(노드 `354:7785`/`354:7899`)은 `HistoryCard`(뱃지 없이 View Course 버튼 1개) 리스트를 보여주며, 둘 다 비어 있으면 같은 빈 상태("No courses created yet.")를 공유함. 두 카드의 View Course 버튼은 `/course/[id]`로 연결됨(Edit Schedule/FAB는 여전히 미연결). `/course/[id]`(Figma 노드 `358:10841`)는 뒤로가기+"Course name" 헤더 + 지도/삭제 아이콘(미연결) + 세로 점선 타임라인(원형 마커 + `ui/spot-card.tsx` 재사용 + 정거장 사이 거리) — 코스-스팟 연결 API가 없어 `SPOTS[0]`을 3번 반복하는 정적 스텁 |
