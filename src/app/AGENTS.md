<!-- Parent: ../AGENTS.md -->
<!-- Generated: 2026-08-01 | Updated: 2026-08-23 -->
...
| `login/`        | 로그인 플로우 라우트 — `/login`, `/login/terms`, `/login/email`, `/login/reset/email`. 회원가입(`/login/email`)과 비밀번호 재설정(`/login/reset/email`)은 각각 **단일 라우트 + 내부 `step` state** 오케스트레이터입니다 — `course/course-create-screen.tsx`와 같은 패턴(`components/AGENTS.md` 참고)으로, 이전엔 단계마다 라우트를 분리(`/login/code`, `/login/password`, `/login/nickname`, `/login/reset/code`, `/login/reset/password`)하고 `push`/`replace` 히스토리 관리 + draft Context + "새로고침하면 데이터 날아감" 가드까지 뒀었는데, 그 구조가 낳은 버그(새로고침 후 뒤로가기 시 유령 히스토리 엔트리로 계속 튕김, 제출 성공 시 가드와 성공 네비게이션이 경쟁해 방금 끝난 가입이 다시 이메일로 튕기는 레이스 등)를 겪고 나서 라우트를 아예 안 쪼개는 쪽으로 다시 설계했습니다. `email`/`code`/`password`/`nickname` 각 단계 값(email/password 또는 email/resetToken)은 오케스트레이터의 평범한 `useState`로만 있고 어디에도 영속화하지 않습니다 — 새로고침하면 그냥 첫 단계(email)부터 다시 시작(부분 복구 시도 자체가 없어서 더 단순함). 헤더의 뒤로가기 버튼은 `step > 첫 단계면 이전 단계로, 아니면 router.back()`(직전 화면인 `/login/terms` 또는 `/login`으로) — `course-create-screen.tsx`와 동일한 분기입니다. 화면 컴포넌트는 `src/components/login/`에 있고 `headerTitle`/`heading`/`email`/`onBack`/`onSubmit`(또는 `onConfirm`/`onResend`)/`onNext` props만 받는 순수 프레젠테이션 컴포넌트입니다(`components/AGENTS.md` 참고) — 라우팅도 다음 단계가 뭔지도 모르고, 오케스트레이터가 `onNext`로 무엇을 할지(다음 단계로 `setStep` 할지, `router.replace`로 위저드를 나갈지) 전부 결정합니다. `/login/email`(회원가입)과 `/login/reset/email`(재설정)의 두 `page.tsx`는 함수(콜백)를 화면 컴포넌트 prop으로 넘겨야 해서 `"use client"`입니다. |
