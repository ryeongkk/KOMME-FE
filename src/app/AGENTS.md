...
| `login/`        | 로그인 플로우 라우트 — `/login`, `/login/terms`, `/login/email`, `/login/code`, `/login/password`, `/login/nickname`, `/login/reset/email`, `/login/reset/code`, `/login/reset/password`. `layout.tsx`가 `SignupDraftProvider`/`PasswordResetDraftProvider`(`lib/signup-draft.tsx`/`lib/password-reset-draft.tsx`)로 `/login/*` 전체를 감싸 회원가입·재설정 위저드 상태를 in-memory Context로 유지함(라우트 이동해도 레이아웃은 안 죽음 — sessionStorage 대신 쓰는 이유는 `components/AGENTS.md` 참고). 화면 컴포넌트는 `src/components/login/`에 있고 각 `page.tsx`는 그걸 렌더링만 함 — `reset/` 쪽 3개는 회원가입과 같은 화면 컴포넌트를 다른 문구/다음 라우트/`onSubmit` prop으로 재사용함 (`components/AGENTS.md` 참고). `email`/`code`/`password`/`nickname`/`reset/*` 7개 `page.tsx`는 전부 `onSubmit`/`onConfirm`/`onResend`에 클로저(draft Context 저장, `lib/api/auth.ts` 호출)를 넘겨야 해서 `"use client"`입니다 — Server Component는 함수를 Client Component prop으로 못 넘김. 첫 단계(`email`/`reset/email`)를 뺀 나머지 6개 `page.tsx`(`code`, `password`, `nickname`, `reset/code`, `reset/password`)는 각자 필요한 draft 필드(`code`/`password`는 `email`, `nickname`은 `email`+`password`, `reset/code`는 `email`, `reset/password`는 `resetToken`)를 `useRequiredSignupDraft(...)`/`useRequiredResetDraft(...)`(`lib/signup-draft.tsx`/`lib/password-reset-draft.tsx`)로 가드합니다 — 새로고침 등으로 in-memory draft가 비면 해당 필드가 채워질 때까지 기다리지 않고 즉시 `/login/email`·`/login/reset/email`로 `router.replace`하고 그 사이엔 `null`을 렌더링합니다. 이 훅은 `Required<Pick<Draft, K>>`를 반환해서 호출부가 `!` 단언 없이 바로 `draft.email` 등을 쓸 수 있습니다 — 이전엔 `nickname`/`reset/password`의 `onSubmit`에서 필드 누락을 사후 감지해 에러를 던지는 방식(`SessionExpiredError`)이었는데, 사용자가 이미 소용없어진 나머지 단계(비밀번호 입력 등)를 다 마친 뒤에야 에러를 보게 되는 문제가 있어서 이 방식으로 교체했습니다 |
